...
2. 輸入時間上限（秒），按開始／繼續訓練。同一回合一直轉動，沒有步數上限，不會每隔幾步把魔方重設。只有解出、到時間上限或手動停止才結束本次執行。
3. 時間到、首次解出或按停止並保存時，系統保存神經權重、活動快照及本輪變化。時間在每個動作之間檢查，最後寫檔和製作回放可能稍晚完成；看到完成狀態後再關閉。
4. 再按開始會接續相同資料集、魔方大小與深度的 checkpoint。未解出時從最後保存的魔方位置、回合步數、神經動態與亂數狀態繼續，時間上限重新計算；已解出時才開始下一個打亂狀態，保留學到的權重並重設短期神經動態。
...
outputs/live/sessions/malecns_cube3_depth1/
  checkpoint.npz                  目前魔方、最新模型、神經狀態、亂數與回合進度
...
checkpoint 會定期保存（預設約 10 秒，在動作之間檢查）並在正常停止時再保存，採臨時檔完成後原子替換。未完成的回合會保留已學到的突觸權重、感覺／動作介面、膜電位、突觸電流、活動與學習 trace、亂數狀態，以及最近拜訪的魔方狀態；接續從完整決策的邊界開始。更新的是既有突觸的權重，不是新增神經元或生長軸突。

舊版 checkpoint 只保存最初的打亂狀態，無法還原當時最後一步。第一次載入這類舊檔時，系統先以 `checkpoint-v1-*.npz` 備份原檔並保留模型權重，再從原始打亂狀態開始連續回合；若舊回合已解出則換新魔方。頁面會顯示遷移通知，之後新保存的檔案便會完整接續目前狀態。舊版的嘗試統計保存在 `legacy_totals`，新版完成回合數只計算已解出的魔方。

訓練區會顯示「本回合累積步數」與「本次啟動轉動」：前者包含之前暫停的進度，後者只計算這次設定時間內的轉動。同一回合的累積獎懲也跨啟動保留；右下明細顯示最新一步各項分數。

## 連續回合的獎懲

每一步都從轉動前後的貼紙狀態計算獎懲，並更新既有的可塑性突觸。規則固定如下：

| 項目 | 分數 |
| --- | --- |
| 同面同色配對改善 | 轉動後的配對分數減去轉動前的分數；權重 `1.0` |
| 轉動成本 | 每一步 `−0.002` |
| 立即反轉 | 本步剛好撤銷前一步時，額外 `−0.01` |
| 重訪狀態 | 回到最近 256 個狀態中的任一狀態時，額外 `−0.02` |
| 解出 | 六面各自全同色時，額外 `+1`，然後停止 |

所有項目相加得到本步總獎懲；時間到或手動停止不額外扣失敗分。同面同色配對分數是每個面內同色貼紙配對數，占所有面內貼紙配對數的比例；範圍為 0 到 1，解出時等於 1。它不會讀取解答、逆打亂序列或最短解距離，必要的暫時退步也可能扣分。單純繞一圈回到原狀不會累積配對改善獎勵，還需付出步數與可能的重訪成本。

這是工程獎懲設計，不是果蠅天然獎勵系統的重建。它與研究 benchmark／curriculum 的有限步數、純 solved reward 協定不同，也取代先前限時模式的「走滿步數扣分後重試」規則；歷史結果不能直接當作這版的學習效果。時間更長仍不保證解出或泛化能力提升。

## 如何看三個視覺區域

魔方區依模型實際選擇的面轉動，右側神經區對應同一筆動作記錄。即時訓練可能在兩次畫面更新之間走多步，頁面顯示收到的精確最新狀態；訓練速度不被動畫播放速度限制。停止後可查看保存權重所產生的凍結回放。回放是另行錄製的有限片段，可能包含多個回放回合；這不代表連續訓練曾重設魔方。
